<template>
  <v-expansion-panel>
    <v-expansion-panel-title>
      <v-icon class="mr-2">{{
        teamType === "homeTeam" ? "mdi-home" : "mdi-account-group"
      }}</v-icon>
    ...
    <v-expansion-panel-text style="padding-left: 0px; padding-right: 0px">
      <div class="team-edit-form">
        <div class="row mb-3">
          <div class="col-md-12">
            <v-text-field
              :model-value="team.teamName"
              @update:model-value="updateField('teamName', $event)"
              label="Team Name *"
        ...
        <template v-if="team.innings">
          <InningsSection
            v-for="(inning, inningIndex) in team.innings"
            :key="inningIndex"
            :inning="inning"
            :teamTotalScore="team.totalScore"
            @update:inning="updateInning(inningIndex, $event)"
            @add-player="$emit('add-player', inningIndex)"
            @remove-player="
              (playerIndex) => $emit('remove-player', inningIndex, playerIndex)
            "
            @add-bowler="$emit('add-bowler', inningIndex)"
            @remove-bowler="
              (bowlerIndex) => $emit('remove-bowler', inningIndex, bowlerIndex)
            "
            @add-fielder="$emit('add-fielder', inningIndex)"
            @remove-fielder="
              (fielderIndex) =>
                $emit('remove-fielder', inningIndex, fielderIndex)
            "
          />
        </template>
      </div>
