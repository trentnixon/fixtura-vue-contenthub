<template>
  <div v-if="!localMeta" class="text-center py-4">
      ...
      <v-col cols="12" sm="6" class="pa-4">
        <v-card class="py-2 px-1 elevation-0 bg-surface-lighten1 rounded-md mt-4">
          ...
          <v-card class="pa-4 elevation-0 bg-surface rounded-md">
            <MediaImageInput :value="localMeta.Video.HeroImage" @update="(val) => (localMeta.Video.HeroImage = val)" />
            ...
            <FormRowTwoItems>
              ...
              <template #form-element>
                ...
                <TextInput label="Organisation Name" :value="localMeta.Club.Name"
                  @update="(val) => (localMeta.Club.Name = val)" />
              </template>
            </FormRowTwoItems>
            <FormRowTwoItems>
              <template #description>
                <ColorInput label="Primary Color" :value="localMeta.Video.Theme.primary"
                  @update="(val) => (localMeta.Video.Theme.primary = val)" />
              </template>

              <template #form-element>
                <!-- Form element content here -->
                <ColorInput label="Secondary Color" :value="localMeta.Video.Theme.secondary"
      ...
      <v-col cols="12" sm="6" class="pa-4">
        <v-card class="py-2 px-1 elevation-0 bg-surface-lighten1 rounded-md mt-4">
          ...
          <v-card class="pa-4 elevation-0 bg-surface rounded-md">
            <v-row>
              <v-col cols="12" class="px-0">
                <div class="pa-2 d-flex justify-start content-center align-center">
                  <TextInput label="Opening Video Title" :value="localMeta.Video.Title"
            ...
            <FormRowTwoItems>
              <template #description>
                <TextInput label="Section Title" :value="localMeta.Video.TitleSplit[0]"
                  @update="(val) => (localMeta.Video.TitleSplit[0] = val)" />
              </template>

              <template #form-element>
                <!-- Form element content here -->
                <TextInput label="Section Subtitle" :value="localMeta.Video.TitleSplit[1]"
