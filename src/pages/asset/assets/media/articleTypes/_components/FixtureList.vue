  ...
  <div>
    ...
    <div v-else-if="fixtures.length === 0" class="text-center pa-8">
      ...
      <p class="text-body-2 text-grey">
        No fixture data is available for this article.
      </p>
    </div>

    <!-- No results from filter -->
    <div
      v-else-if="filteredAndSortedFixtures.length === 0"
    ...
    <div
      v-else
      class="fixture-list"
      :style="{ maxHeight: minHeightStyle, overflowY: 'auto' }"
    >
      <template
        v-for="(item, index) in filteredAndSortedFixtures"
      ...
      >
        ...
        <v-divider
          v-if="index < filteredAndSortedFixtures.length - 1"
          class="my-2"
        />
      </template>
    </div>
...
defineProps<{
  ...
  filteredAndSortedFixtures: Array<{
