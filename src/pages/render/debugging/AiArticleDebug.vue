    ...
    <v-container fluid class="pa-0">
        ...
        <div v-if="aiArticlesByRenderID.length && !loading" class="mt-4">
            <v-expansion-panels flat>
                ...
                <v-expansion-panel>
                    <v-expansion-panel-title>AI Articles with Errors</v-expansion-panel-title>
                    <v-expansion-panel-text>
                ...
                <v-expansion-panel v-for="(articles, type) in groupArticlesByAssetType" :key="type">
                    <v-expansion-panel-title>Asset Type: {{ type }}</v-expansion-panel-title>
                    <v-expansion-panel-text>
                        <ul>
                ...
                <v-expansion-panel v-for="(articles, category) in groupArticlesByAssetCategory" :key="category">
                    <v-expansion-panel-title>Asset Category: {{ category }}</v-expansion-panel-title>
                    <v-expansion-panel-text>
                        <ul>
                            <li v-for="article in articles" :key="article.id">
                                ID: {{ article.id }} | Name: {{ article.attributes.Name }}
