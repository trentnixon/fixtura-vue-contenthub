  ...
  <div v-else>
    ...
    <ul>
      <li v-for="download in getFilteredDownloads" :key="download.assetId">
        Download ID: {{ download.assetId }} (Category:
        {{ download.assetCategoryID }})
      </li>
    </ul>
    ...
    <ul>
      <li v-for="fullDownload in getFullDownloads" :key="fullDownload?.id">
        ...
        <strong>Name:</strong> {{ fullDownload?.attributes?.Name || "N/A"
        }}<br />
        <strong>Type:</strong>
        {{
          fullDownload?.attributes?.asset_category?.data?.attributes?.Name ||
          "Unknown"
        }}
      </li>
    </ul>
    ...
    <ul>
      <li v-for="fullAiArticle in getFullAiArticles" :key="fullAiArticle?.id">
        ...
        <strong>Name:</strong> {{ fullAiArticle?.attributes?.Name || "N/A"
        }}<br />
        <strong>Category:</strong>
        {{
          fullAiArticle?.attributes?.asset_category?.data?.attributes?.Name ||
          "Unknown"
        }}
      </li>
    </ul>
