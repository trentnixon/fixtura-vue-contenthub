<template>
  <v-container fluid class="pa-0">
    <HeaderSection />
    <AnnouncementBanner
      :enabled="showAnnouncementBanner"
    ...
    >
      ...
      <div class="d-flex align-center">
        ...
        <span class="text-caption"
          >We have received your rerender request and will review it
          shortly.</span
        >
      </div>
    </v-alert>
    ...
    <v-dialog v-model="showRerenderDialog" max-width="500px">
      <v-card>
        <v-card-title class="text-h5"> Request a Rerender </v-card-title>
        <v-card-text v-if="!showSuccessMessage">
          <p class="mb-4">
            To request a rerender of this bundle please let us know what was
            wrong with this addition.
          </p>
          <v-select
            v-model="selectedRerenderReason"
            :items="rerenderReasons"
            label="Select reason"
            variant="outlined"
        ...
        <v-card-text v-else>
          <p class="mb-2">
            Your request has been sent to us, we will review and contact you
            with a new render or clarification shortly.
          </p>
  ...
  try {
    ...
    const errorMessage =
      error instanceof Error
        ? error.message
        : "Failed to submit rerender request";
