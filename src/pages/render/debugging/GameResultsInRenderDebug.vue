    ...
    <v-container fluid class="pa-0">
      ...
      <div v-if="gameResultsInRenderByRenderID.length && !loading" class="mt-4">
        <v-expansion-panels flat>
          <!-- Grouped by Sport -->
          <v-expansion-panel v-for="(results, sport) in groupGameResultsBySport" :key="sport">
            <v-expansion-panel-title>{{ sport }} Game Results</v-expansion-panel-title>
            <v-expansion-panel-text>
              <ul>
                <li v-for="result in results" :key="result.id">
                  ID: {{ result.id }} | Meta Data: {{ result.attributes.game_meta_datum?.data?.id }}
                </li>
              </ul>
            </v-expansion-panel-text>
          </v-expansion-panel>
