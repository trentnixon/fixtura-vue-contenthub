    ...
    <div>
      ...
      <ConfirmationModal v-model="showConfirmationDialog" :title="`Confirm ${buttonText}`" :persistent="isPending"
        ...
        <p v-if="articlePhase === 'initial' || articlePhase === 'postPending'">
          Are you sure you want to request a new AI Ladder Summary article? This will generate a fresh article based on
          the current data.
        </p>
      ...
      <!-- Article Display -->
      <LadderDisplay :articleStatus="articleStatus" :formattedArticles="formattedArticles" :isRequesting="isPending"
...
async function onRequestWriteup() {
  try {
    ...
    // Note: Using weekend article action for now - replace with Ladder-specific action when available
