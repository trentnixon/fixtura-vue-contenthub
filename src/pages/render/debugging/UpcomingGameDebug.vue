  ...
  <v-container fluid class="pa-0">
    ...
    <div v-if="groupUpcomingGamesBySport && !loading" class="mt-4">
      <v-expansion-panels flat>
        <!-- Grouped by Sport -->
        <v-expansion-panel
          v-for="(games, sport) in groupUpcomingGamesBySport"
          :key="sport"
        >
          ...
          <v-expansion-panel-text>
            <ul>
              <li v-for="game in games" :key="game.id">
                ...
                {{
                  game.attributes.game_meta_datum?.data?.attributes?.teamHome
                }}
                vs
                {{
...
import { useUpcomingGameData } from "@/pages/render/composables/OLD_useUpcomingGameData";
