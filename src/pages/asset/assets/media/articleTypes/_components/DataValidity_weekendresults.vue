<template>
  <div
    class="w-100 bg-grey-lighten-4 pt-3 pt-sm-4 px-2 px-sm-4 pb-2 mt-4"
    style="border-top: 1px solid rgba(0, 0, 0, 0.08)"
  >
    <div
      v-if="isSavingFixtures"
    ...
    <div
      v-else-if="validationResults.length === 0"
      class="text-caption text-grey"
    >
      No fixtures found
    </div>
    <div v-else>
      <div
        class="d-flex flex-column flex-sm-row align-center align-sm-start justify-center justify-sm-space-between mb-1"
        style="gap: 8px"
      >
        <div
          class="d-flex align-center justify-center flex-sm-justify-start flex-wrap"
          style="gap: 4px"
        >
          ...
          <span class="text-caption text-grey">
            ...
            {{ validationResults.length === 1 ? "fixture" : "fixtures" }})
          </span>
        </div>
        <!-- Dialog Button - Only show if score is not 100 -->
        <v-btn
          v-if="overallScore < PERFECT_SCORE"
          color="warning"
          variant="tonal"
          size="small"
          @click="showMissingDataDialog = true"
          class="text-caption"
          density="comfortable"
          :aria-label="`View missing data for ${
            fixturesWithMissingData.length
          } ${fixturesWithMissingData.length === 1 ? 'fixture' : 'fixtures'}`"
        >
      ...
      <div
        class="text-caption text-grey text-center text-sm-left mt-1 d-none d-sm-block"
      >
        {{ validitySummary }}
      </div>
    </div>
    ...
    >
      <v-card :class="isMobile ? 'ma-2' : ''" style="max-height: 90vh">
        ...
        <v-card-text class="pa-3 pa-sm-4">
          <div
            v-for="(result, index) in fixturesWithMissingData"
            :key="index"
            class="mb-4"
          >
            <p
            ...
            >
              {{ result.homeTeamName || "Home Team" }} vs
              {{ result.awayTeamName || "Away Team" }}
            </p>
            <div class="pa-2 bg-error-lighten-5 rounded">
              <p class="text-caption font-weight-bold text-error mb-1">
                Missing Data:
              </p>
              <ul
                class="text-caption text-error mb-0 pl-4"
          ...
          <div
            v-if="fixturesWithMissingData.length === 0"
          ...
          >
            ...
            <p class="text-body-1 text-success">
              All fixtures have complete data!
            </p>
          </div>
          ...
          >
            <div class="mb-2">
              <div
                class="d-flex align-center flex-wrap justify-center justify-sm-start"
                style="gap: 4px"
              >
                <v-icon size="12">mdi-information</v-icon>
                <span class="text-caption text-grey-darken-1"
                  >To fix: Click the</span
                >
                <v-icon size="20" color="success" class="flex-shrink-0"
                  >mdi-pencil-box</v-icon
                >
                <span class="text-caption text-grey-darken-1"
                  >button, find the fixture, and add the missing data.</span
                >
              </div>
            </div>
            <div class="d-flex align-start" style="gap: 4px">
              ...
              <span class="text-caption text-grey-darken-1">
                <strong>Note:</strong> Missing data in the prompt may cause
                unexpected results in AI-generated articles.
              </span>
            </div>
          </div>
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" @click="showMissingDataDialog = false"
            >Close</v-btn
          >
        </v-card-actions>
      </v-card>
...
const fixturesWithMissingData = computed(() => {
  return validationResults.value.filter(
    (result) => result.score < PERFECT_SCORE
  );
});
...
function validateFixtures() {
  ...
  setTimeout(() => {
    try {
      ...
      if (
        !articleDataForPrompt ||
        !Array.isArray(articleDataForPrompt) ||
        articleDataForPrompt.length === 0
      ) {
        validationResults.value = [];
        return;
      }
        ...
        try {
          ...
          const countPlayersInTeam = (team: typeof homeTeam) => {
            ...
            return team.innings.reduce((count, inning) => {
              return (
                count +
                (inning.battingOrder && Array.isArray(inning.battingOrder)
                  ? inning.battingOrder.length
                  : 0)
              );
            }, 0);
          };
          ...
          totalPlayerCount =
            countPlayersInTeam(homeTeam) + countPlayersInTeam(awayTeam);
          ...
          const score = Math.round(
            checks.filter(Boolean).length * POINTS_PER_CHECK
          );
...
const overallScore = computed(() => {
  ...
  const totalScore = validationResults.value.reduce(
...
const validitySummary = computed(() => {
  ...
  if (score === 100)
    return "Excellent - All data is complete and ready for article generation.";
  if (score >= 91)
    return "Very good - Data is nearly complete with minimal issues.";
  if (score >= 81)
    return "Good - Most data is present, minor improvements needed.";
  if (score >= 71)
    return "Above average - Some data missing, review recommended.";
  if (score >= 61) return "Average - Several data points need attention.";
  if (score >= 51)
    return "Below average - Multiple data fields require updates.";
  if (score >= 41) return "Low - Significant data gaps need to be addressed.";
  if (score >= 31) return "Very low - Major data issues present.";
  if (score >= 21)
    return "Poor - Critical data missing, urgent action required.";
  if (score >= 11)
    return "Very poor - Most data is missing, article generation may be limited.";
...
const scoreTooltipText = computed(() => {
  ...
  const scoreInfo = `Score: ${overallScore.value}/100 (average across ${
    validationResults.value.length
  } ${validationResults.value.length === 1 ? "fixture" : "fixtures"})`;
